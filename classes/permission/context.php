...
class context {
    ...
    public function isAnyKindOfModerator($checkRoleSwitch = false){
        if($checkRoleSwitch){
            try{
                global $USER;
                $switched = $USER->access['rsw'][$this->_context->path];                
                if(!is_null($switched) && is_int($switched)){
                    $roles = get_all_roles();
                    foreach($roles as $role){
                        if(intval($role->id) === intval($switched)){

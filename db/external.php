...
class mod_write_external extends external_api
{
    ...
    private static function getWidgetStructure(): external_single_structure
    {
        return new external_single_structure(array(
            ...
            'uuid' => new external_value(PARAM_RAW, 'The unique UUID, should be generated by the frontend if not yet set.'),
    ...
    public static function saveDashboard($cmid, $sentWidgets)
    {
        ...
        foreach ($sentWidgets as $sentWidget) {
            $existingWidget = mod_write\etherpad\client::findByAttributeInArray('uuid', $sentWidget['uuid'], $existingWidgets);

            if ($existingWidget) {
                $sentWidget['id'] = $existingWidget->id;
        ...
        foreach ($existingWidgets as $w) {
            $stillThere = mod_write\etherpad\client::findByAttributeInArray('uuid', $w->uuid, $widgetsToUpdate);
            if (!$stillThere) {
                $widgetsToDelete[] = $w;
            }
        }
